...
def acfree_post_process(input_data):
    ...
    print(f'num of boxes before nms --> {len(scores)}')
    ...
    print(f'num of boxes after nms --> {len(scores)}')
...
if __name__ == '__main__':
    ...
    # for i, outputs_save in enumerate(outputs):
    #     outputs_save = np.transpose(outputs_save, (0, 2, 3, 1))
    #     np.savetxt(f'/home/manu/tmp/rknn_outputs_{i}.txt', outputs_save.flatten(), fmt="%f", delimiter="\n")
    # sys.exit(0)

    # post process
    input0_data = outputs[0]  # 1 x 1 x 80 x 80
    input1_data = outputs[1]  # 1 x 1 x 40 x 40
    input2_data = outputs[2]  # 1 x 1 x 20 x 20
    input3_data = outputs[3]  # 1 x 4 x 80 x 80
    input4_data = outputs[4]  # 1 x 4 x 40 x 40
    input5_data = outputs[5]  # 1 x 4 x 20 x 20

    # input_data_save = np.transpose(input5_data, (0, 2, 3, 1))
    # np.savetxt('/home/manu/tmp/rknn_outputs_debug.txt', input_data_save.flatten(), fmt="%f", delimiter="\n")

    input0_data_t = np.transpose(input0_data, (2, 3, 0, 1))  # 80 x 80 x 1 x 1
    input1_data_t = np.transpose(input1_data, (2, 3, 0, 1))  # 40 x 40 x 1 x 1
    input2_data_t = np.transpose(input2_data, (2, 3, 0, 1))  # 20 x 20 x 1 x 1
