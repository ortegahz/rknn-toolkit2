...
RKNN_MODEL = '/home/manu/nfs/tmp/install/rknn_yolov5_demo_Linux/model/RK3588/acfree_mh_s1280.rknn'
# IMG_PATH = '/home/manu/nfs/tmp/install/rknn_yolov5_demo_Linux/model/sylgd_rp.bmp'
IMG_PATH = '/home/manu/nfs/tmp/install/rknn_yolov5_demo_Linux/model/126_1280.bmp'
...
CLASSES = ('head', 'face', 'stand', 'lookback', 'handsup', 'overdesk')


def post_process_face(outputs, img, color=(0, 0, 255)):
    input0_data = outputs[6]  # 1 x c x 160 x 160
    input1_data = outputs[7]  # 1 x c x 80 x 80
    input2_data = outputs[8]  # 1 x c x 40 x 40
    input3_data = outputs[9]  # 1 x 14 x 160 x 160
    input4_data = outputs[10]  # 1 x 14 x 80 x 80
    input5_data = outputs[11]  # 1 x 14 x 40 x 40
    ...
    input3_data_t = np.transpose(input3_data, (2, 3, 0, 1))  # 160 x 160 x 1 x 14
    input4_data_t = np.transpose(input4_data, (2, 3, 0, 1))  # 80 x 80 x 1 x 14
    input5_data_t = np.transpose(input5_data, (2, 3, 0, 1))  # 40 x 40 x 1 x 14

    input3_data_t_b = input3_data_t[:, :, :, :4]  # 160 x 160 x 1 x 4
    input4_data_t_b = input4_data_t[:, :, :, :4]  # 80 x 80 x 1 x 4
    input5_data_t_b = input5_data_t[:, :, :, :4]  # 40 x 40 x 1 x 4
    input3_data_t_kps = input3_data_t[:, :, :, -10:]  # 160 x 160 x 1 x 10
    input4_data_t_kps = input4_data_t[:, :, :, -10:]  # 80 x 80 x 1 x 10
    input5_data_t_kps = input5_data_t[:, :, :, -10:]  # 40 x 40 x 1 x 10
...
def post_process_bhv(outputs, img, color=(0, 255, 0)):
    input0_data = outputs[12]  # 1 x c x 160 x 160
    input1_data = outputs[13]  # 1 x c x 80 x 80
    input2_data = outputs[14]  # 1 x c x 40 x 40
    input3_data = outputs[15]  # 1 x 14 x 160 x 160
    input4_data = outputs[16]  # 1 x 14 x 80 x 80
    input5_data = outputs[17]  # 1 x 14 x 40 x 40
    ...
    if boxes is not None:
        draw(img, boxes, scores, classes + 2, color=color)
...
def post_process_head(outputs, img, color=(0, 255, 0)):
    ...
    input3_data = outputs[3]  # 1 x 14 x 160 x 160
    input4_data = outputs[4]  # 1 x 14 x 80 x 80
    input5_data = outputs[5]  # 1 x 14 x 40 x 40
...
if __name__ == '__main__':
    ...
    ret = rknn.load_onnx(model=ONNX_MODEL,
                         outputs=['/detect/cls_preds.0/Conv_output_0',
                                  ...
                                  '/detect_bhv/cls_preds.0/Conv_output_0',
                                  '/detect_bhv/cls_preds.1/Conv_output_0',
                                  '/detect_bhv/cls_preds.2/Conv_output_0',
                                  '/detect_bhv/reg_preds.0/Conv_output_0',
                                  '/detect_bhv/reg_preds.1/Conv_output_0',
                                  '/detect_bhv/reg_preds.2/Conv_output_0'])
    if ret != 0:
    ...
    # cv2.imwrite('/home/manu/tmp/126_1280.bmp', img)
    ...
    post_process_bhv(outputs, img_1)
